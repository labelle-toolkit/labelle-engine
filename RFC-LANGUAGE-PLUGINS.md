...
- **Honest boundary.** Two pieces of assembler *codegen keyed to a language* are deliberately NOT table-lookup wiring, and stay **named extension points** (future: template-driven, plugin-supplied templates), not silent gaps: (1) typed-binding sidecar generation — the `labelle-components.d.ts` built from the registry; (2) the transpile-invocation codegen — the generated `tsconfig.json`, the emitted-file collision policy, and the `<tool> -p` shape (rev 18 moves the transpile toolchain's *fetch pins* to the manifest, but not this codegen). v1 agnosticism deliberately scopes to collection, declare, embed, native wiring, and toolchain fetch-pins; these two codegen surfaces are the acknowledged residue.
