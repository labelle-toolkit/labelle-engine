...
**Status:** Draft  
**Author:** Alexandre  
**Date:** 2026-07-10 (rev 2 — POC validated: PR #734; rev 3 — single-repo packaging: `labelle-scripting` with language sub-modules; rev 4 — reference bindings: Lua queries, Ruby events; rev 5 — Ruby controllers: script-language domain owners; rev 6 — script-declared components: generate-time codegen, runtime tier for mods; rev 7 — native declaration idioms per language; rev 8 — policy: one language per project, enforced at generate; rev 9 — policy rationale: role-based, pack/mod carve-outs; rev 10 — Zig plugins in script-language projects; rev 11 — script-language packs; rev 12 — TypeScript (QuickJS) and Go (c-archive) join the families; rev 13 — language rides generic plugin params; rev 14 — per-frame allocation idioms: FrameArray, into: reuse; rev 15 — idioms generalized per language; rev 16 — the 2026-07-12 product decisions: purity mandate (100% selected-language games), convention dirs shipped (scripts/, components/, events/), the declare contract as the language-neutral seam, the language-agnostic assembler (capability rows); rev 17 — native languages declare via the same `.declare` capability + the full declare invocation contract (persistent cache dir, hash-and-skip), PR #777; rev 18 — the transpile fetch-pin contract: resolves the toolchain-pin open question to option (b) manifest-with-mandatory-hashes, pins the `.transpile` row schema (version + per-platform package/binary/sha512) and the assembler's fetch-verify-run consume contract, scoping the tsconfig/invocation codegen as an explicit extension point; rev 19 — the last two unspecified categories, closing the §7 move: the EMBED/registration capability (`.kind = .embedded` + `.extensions` = the *authored* source extensions; the assembler's convention-dir copy + `registerScript`/`@embedFile` consume contract, with the untranspiled-source gate generalized to a self-describing capability check) and the NATIVE wiring capability (`.kind = .native` + `.module_root` + `.stage_subdir`; the assembler's source-staging consume contract, build steps already in `.language_builds`), plus the naming of the four generated splice touchpoints — dep-arg, alias+flag, drain tap, controller tick — as the assembler-owned Script-Runtime-Contract-v1 codegen extension point rather than per-language row data — OPEN for the epic owner, #619; rev 20 — the declare↔transpile composition for a language carrying BOTH capabilities: option (b), the assembler transpiles the declaration files BEFORE the declare phase and hands the declare tool the EMITTED output, so the declare tool stays a toolchain-agnostic embedded-VM evaluator (the quickjs declare host is the lua/ruby twin, knows nothing about tsc) — user-ratified, DECIDED, #773)
...
.languages = .{
    .{ .name = "ruby", .extensions = .{"rb"}, .kind = .embedded,
       // rev 19: `.extensions` names the AUTHORED source extensions the
       // convention-dir scan collects. For a non-transpiled embedded
       // language (lua, ruby) that IS the extension `registerScript`
       // embeds; for a transpiled one (typescript) the embedded extension
       // is `.transpile.emits` (rev 18) — authored `.ts`, registered
       // `.js`. See the embed/registration bullet.
    ...
    .{ .name = "rust", .extensions = .{"rs"}, .kind = .native,
       // rev 19 pins the native staging pair:
       .module_root = "mod.rs",                // the file the author places at the convention dir root (crate module root); its absence is a pointed generate error
       .stage_subdir = "native/src/game",      // the PLUGIN-crate-relative dir the game's sources link OVER (replacing the shipped placeholder module); build steps stay in .language_builds
       .declare = .{ .tool = "labelle-declare-rs", .dir = "tools/declare-rs",
                     .events = true } },        // rev 17: native languages declare too — probe is internal to the tool
    .{ .name = "typescript", .extensions = .{"ts"}, .kind = .embedded,
       // rev 18: the transpile capability carries the toolchain FETCH PINS —
       // exact `.version`, a `.fetch_url` template, and one `.platforms` row
       // per (os, arch) naming the registry package, the in-tarball binary
       // path, and a MANDATORY `.sha512`. `.emits` is the transpiled
       // extension. The assembler refuses a hash-less platform row.
       //
       // This mirrors the assembler's REAL shipping tsc integration
       // (`scripting_transpile.zig`): TypeScript 7.0 (GA 2026-07-08) ships
       // the Go-native `tsc` as npm platform packages
       // `@typescript/typescript-<os>-<arch>` — esbuild's distribution shape,
       // a registry tarball whose `package/lib/` holds the native binary.
       // (Pre-7.0 `tsc` was pure JS with no platform binaries — the pin is
       // the Go-native line.) VALUES BELOW ARE ILLUSTRATIVE: the real row
       // pins an EXACT published version (e.g. 7.0.2 — 7.0.0 is not a
       // published platform-package version) and the real per-platform
       // hashes (the assembler ships them today in `TSC_PLATFORMS`).
       .transpile = .{
           .emits = "js",
           .toolchain = "tsc",
           .version = "7.0.2", // exact published pin (illustrative)
           .fetch_url = "https://registry.npmjs.org/@typescript/typescript-{platform}/-/typescript-{platform}-{version}.tgz",
           // `.sha512` is the RAW base64 digest (the npm `dist.integrity`
           // SRI value with its `sha512-` prefix stripped — the assembler's
           // `tarball_sha512_b64` spelling today). The schema is base64, not
           // SRI: a `sha512-`-prefixed value is a malformed row.
           .platforms = .{
               .{ .os = "macos",   .arch = "aarch64", .platform = "darwin-arm64", .binary = "lib/tsc",     .sha512 = "<base64…>" },
               .{ .os = "macos",   .arch = "x86_64",  .platform = "darwin-x64",   .binary = "lib/tsc",     .sha512 = "<base64…>" },
               .{ .os = "linux",   .arch = "x86_64",  .platform = "linux-x64",    .binary = "lib/tsc",     .sha512 = "<base64…>" },
               .{ .os = "linux",   .arch = "aarch64", .platform = "linux-arm64",  .binary = "lib/tsc",     .sha512 = "<base64…>" },
               .{ .os = "windows", .arch = "x86_64",  .platform = "win32-x64",    .binary = "lib/tsc.exe", .sha512 = "<base64…>" },
           },
       } },
       // NO `.declare` for typescript AT REV 18/19 — SUPERSEDED BY REV 20
       // (#773), which adds `.declare = { .tool = "labelle-declare-ts",
       // .dir = "tools/declare-ts", .events = true }` to this row. Still NOT
       // the same thing as the SHIPPED ts d.ts codegen — two distinct
       // directions must not be conflated: (a) the shipped
       // `labelle-components.d.ts` codegen goes registry → editor typings
       // (the honest-boundary extension point below), NOT source → schema;
       // (b) the ts DECLARE-mode tool reading `components/*.ts` → schema is
       // rev 20 / #773. Under rev 20's option (b) the row carries BOTH
       // `.transpile` AND `.declare`: the assembler transpiles the
       // declaration files first and runs the declare tool over the emitted
       // `.js` (the tool is a toolchain-agnostic quickjs evaluator — the
       // lua/ruby twin — see the "Declare-over-transpile ordering" bullet).
},
```

- **Declare invocation contract (rev 17) — native languages declare via the same capability.** A `.declare` row is not embedded-only: a native language (rust #774, crystal #775) carries the identical `.declare = { .tool, .dir, .events }` shape (see the `rust` row above) — **no new field, no per-mechanism discriminant**. The assembler stays agnostic because the mechanism is *internal to the tool exe*: it builds the declare tool once via `zig build <tool>` in the plugin package (the same path used for every language's runner), then runs that exe passing the declaration files (`components/*.<ext>` + `events/*.<ext>`) and a **persistent per-project cache directory**, and reads the schema JSON from stdout. What the tool does with the cache dir is opaque to the assembler — an embedded VM ignores it; a native probe uses it as a cargo `target-dir` so an incremental re-extract is warm (~0.4s vs a ~1.4s cold build). Two invariants keep iteration cheap and the assembler language-blind: (1) the assembler **hashes the declaration inputs AND the resolved declare-tool identity, and skips invoking the tool entirely when both are unchanged** — editing a gameplay `scripts/*.rs` never triggers extraction (the hash is scoped to the declaration dirs, not all sources), but a scripting **pin bump / plugin checkout change rebuilds a different tool exe and MUST re-extract** even when the declaration files are byte-identical, or a plugin upgrade would silently reuse the old schema. Folding the built tool binary's bytes into the skip key delivers both (shipped: labelle-assembler#622); (2) the assembler never learns "rust" or "cargo" — a compile-and-run probe is just an exe that happens to shell a compiler. The litmus test still holds: a native declaring language adds a `tools/declare-<lang>` exe + a `.declare` row, **zero assembler changes**.
- **Transpile fetch-pin contract (rev 18) — decides the toolchain-pin open question: option (b).** The transpile capability is narrower than declare: unlike a declare tool (whose whole mechanism is internal to an exe), the transpile phase is mostly assembler-owned *codegen keyed to the toolchain* — the generated `tsconfig.json`, the `.ts`↔`.js` same-stem collision gate, the stem-strip registration, the `<tool> -p <tsconfig>` invocation. Those stay assembler-owned (see the honest boundary below). What moves to the manifest is exactly the **third-party fetch pins** the trust-model bullet named — the one non-agnostic table left in assembler source. The `.transpile` row carries, per the sketch above: an exact `.version`, a `.fetch_url` template with `{platform}`/`{version}` placeholders, and a `.platforms` list — one row per `(os, arch)` giving the registry `.platform` package suffix, the in-tarball `.binary` path (`lib/tsc` / `lib/tsc.exe`), and a **mandatory `.sha512`** (npm `dist.integrity`, standard base64). The assembler's generic consume contract: (1) select the `.platforms` row matching the host `(os, arch)` — no match is a pointed generate error naming the host and the row's supported set, never a silent skip; (2) **refuse a platform row without a `.sha512`** — a hash-less fetch pin fails generate up front ("agnosticism with integrity": the assembler still owns *whether* a byte is trusted, the manifest owns *what* to fetch). `.sha512` is the **raw base64** digest (the npm `dist.integrity` SRI value with its `sha512-` prefix stripped — the assembler's `tarball_sha512_b64` spelling); a `sha512-`-prefixed value is a malformed row, not silently accepted; (3) fetch `.fetch_url` (placeholders filled), **verify the sha512 before extraction**, extract `.binary` into the assembler's shared toolchain cache. The cache identity **must bind the hash**, not just `<toolchain>/<version>/<platform>` — otherwise a second manifest reusing that tuple with a different URL or `.sha512` (or the same project after a repin) would run a previously-extracted binary that was never proven against the current pin, defeating the manifest-owned hash contract across projects. Either key the cache path by `.sha512` too, or **re-verify the cached bytes against the current `.sha512` before running them**; (3a) **extract archive-safely** — a valid sha512 authenticates the *whole tarball* but does not stop a malicious/buggy archive's path-traversal (`../`), absolute-path, symlink, or hardlink entries from escaping the cache dir. Extract into a temporary dir with an archive-safe policy (reject any entry resolving outside the target; validate the row's `.binary` is a normal file within it), then **atomically** promote it into the shared cache; (4) run the still-assembler-owned tsconfig-codegen + `<binary> -p <tsconfig>` pipeline against the resolved binary. The assembler learns nothing tsc-specific beyond "a toolchain identified by `.toolchain`, fetched-and-verified per its rows" — a second transpiled language (a future `.transpile` for another emitter) adds a row + hashes, **zero assembler fetch-table changes**. The version-bump ritual (re-pin every platform hash) moves from assembler source to the manifest, where the toolchain's owner already tracks it. NOTE the residue: the tsconfig contents, the emitted-file collision policy, and the `-p` invocation are NOT in the row — they are assembler codegen the row does not attempt to describe, and generalizing them (a plugin-supplied transpile-invocation template) is explicitly out of v1, tracked with the binding-codegen extension point below.
- **Declare-over-transpile ordering (rev 20) — how a `.transpile` language declares (#773, DECIDED: option (b)).** revs 17–19 left one interaction unspecified: when a row carries BOTH `.declare` (rev 17) and `.transpile` (rev 18) — the typescript case, once #773 adds its declare tool — in which order do the two phases run, and what does the declare tool receive? Two shapes were on the table: **(a)** the declare tool is toolchain-AWARE — it internalizes the transpile the way the native probes internalize their compiler (rev 17), reading authored `components/*.ts` and shelling tsc itself; or **(b)** the assembler transpiles the declaration files FIRST and hands the declare tool the EMITTED output, keeping the tool toolchain-agnostic. **Ratified: option (b)** (user directive). For a language whose row carries `.transpile`, the assembler transpiles its declaration files (`components/*.<ext>` + `events/*.<ext>`) into the per-project declare cache dir BEFORE the declare phase, then invokes `.declare.tool` over the EMITTED files (the `.transpile.emits` extension). The declare tool is a pure embedded-VM evaluator: the quickjs declare host is the **lua/ruby twin** — it evals each emitted-`.js` declaration module against a `Labelle` declare stub (`component`/`event` record; every other call a sentinel) and prints the same schema JSON the lua runner prints for `.lua` and the ruby runner for `.rb` — and it knows NOTHING about tsc: no toolchain fetch, no tsconfig, no `-p`. This keeps rev-18's boundary intact — **the assembler owns ALL tsc** (the transpile fetch-pin, the tsconfig codegen, the invocation) — and the transpile the assembler already runs for `scripts/*.ts` simply runs earlier and over the declaration dirs too. Why (b) over (a): (a) would smuggle a second copy of the whole tsc integration (fetch-verify + tsconfig + invocation) into the declare tool, splitting rev-18's single tsc boundary in two and re-teaching a plugin-shipped exe the supply-chain fetch the assembler was just made to own; (b) reuses the transpile pipeline verbatim and leaves the declare tool as language-introspection only — the same division the interpreted runners already have. Consequences: (1) the assembler's phase order becomes **collect → (transpile declaration files, for `.transpile` rows) → declare → (transpile gameplay scripts) → …**, pulling the idempotent, shared-cache toolchain fetch ahead of declare (it runs once, warming the cache the later gameplay-script transpile reuses); (2) the declare tool's generic argv contract (rev 17: `--cache-dir <dir> <file…>`) is UNCHANGED — under (b) the `<file…>` are the emitted `.js`, and the embedded VM ignores `--cache-dir` exactly as the lua/ruby runners do (an embedded VM has no build workspace to warm); (3) the skip-key invariants (rev 17) still hold — the declare inputs the assembler hashes are the emitted files (a declaration edit re-transpiles then re-extracts; a pin/checkout change rebuilds a different tool exe and re-extracts); (4) the litmus test holds — a second transpiled-AND-declaring language adds a `.transpile` row + a `.declare` row + a `tools/declare-<lang>` embedded-VM exe, **zero assembler changes**, because "transpile, then declare over the emitted output" is a generic capability COMPOSITION the assembler performs whenever a row carries both, not a typescript special case. The rev-18/19 typescript-row sketch's "NO `.declare` for typescript" note (below) is **superseded**: #773 adds `.declare = { .tool = "labelle-declare-ts", .dir = "tools/declare-ts", .events = true }` to the row, and the `.transpile` + `.declare` pair is exactly what triggers the transpile-before-declare composition above.
- **Embed/registration contract (rev 19) — the embedded-VM family's row + consume contract.** This is the category the assembler owns today as `EMBED_LANGUAGES` (`scripting_splice.zig`): the rows `{ .language, .extension, .transpile_gap? }` that say which extension embeds as a script and drive the copy → `registerScript("<stem>", @embedFile("<dir>/<stem><ext>"))` registration. Under capability rows an **embedded** language is a row with `.kind = .embedded` and `.extensions` — nothing more; there is no separate embed table.
  - **Row schema.** `.extensions` is a list of the **authored** source extensions the assembler's convention-dir scan collects (the union across the list). It is deliberately NOT "the extension that embeds": for a non-transpiled VM (lua, ruby) the authored extension *is* the embedded one, but for a transpiled VM (typescript) the author writes `.ts` while the runtime evaluates `.js`. The two are reconciled by the rev-18 `.transpile`: **the registered/embedded extension is `.transpile.emits` when the row carries `.transpile`, else `.extensions` verbatim.** This dissolves today's awkward split, where the shipped `EmbedLanguage.extension` stores the *runnable* `.js` and a side `TranspileGap.source_extension` stores the *authored* `.ts` — one field with a derived-emitted rule replaces both.
  - **Three forks for the epic owner.** (A1, spelling) The shipped table writes extensions WITH a leading dot (`".rb"`, matching `ConventionDir.extension = ".zig"` and the `std.mem.endsWith(name, ext)` scan), while the rev-16/18 sketches write them without (`"rb"`, `.emits = "js"`). **Recommend: pin the leading-dot form (`".rb"`)** — it matches every extension already spelled in a manifest and the scan comparison, and avoids a normalize step; the sketch's dotless spelling was shorthand. (A2, arity) `.extensions` plural vs a singular `.extension`. Every shipped language is 1:1 today, so singular would suffice — but **recommend plural**, because it is free forward-compat (a VM that legitimately accepts more than one source suffix, e.g. `.mjs`+`.js`) and the scan already unions a set. (A3, the untranspiled-source gate) Today `rejectUntranspiledScripts` hard-codes the `.ts`→#586 gap. **Recommend generalizing it to a self-describing capability check**: authored files whose extension has no runnable path in the **pinned** manifest's row — neither directly runnable (`.kind = .embedded` with that extension and no `.transpile`) nor emitted by a `.transpile` — fail generate with a pointed error naming the pin, exactly the property the declare/transpile capabilities already have. The hard-coded `#586` seam then dissolves the same way `min_pin` does below.
  - **Assembler consume contract (generic, no per-language branch).** For a `.kind = .embedded` splice: (1) copy/link the row's convention dir and scan the `.extensions` stems (`scanner.linkAndScan`); (2) run the declare phase iff the row carries `.declare` (rev 17); (3) if the row carries `.transpile`, run the rev-18 transpile pipeline, and register the `.transpile.emits` stems — else register the `.extensions` stems directly; (4) emit one `registerScript("<stem>", @embedFile(...))` per registered stem before `Controller.setup`. Steps 1–4 read only row fields — the litmus `python` embed row (`.kind = .embedded, .extensions = .{".py"}`) drives all four with zero assembler changes.
- **Native wiring contract (rev 19) — the native-compiled family's row + consume contract, completing the move.** This is the category the assembler owns today as `NATIVE_LANGUAGES` (`scripting_splice.zig`): the rows `{ .language, .extension, .stage_subdir, .module_root }` that stage the game's convention-dir sources over the plugin crate's placeholder module (`stageNativeSources`), which the plugin's `.language_builds` steps then compile and link. The build **steps** already moved (rev 16 / engine#741, `.language_builds`); rev 19 moves the **staging geometry** that is still hard-coded.
  - **Row schema.** A **native** language is a row with `.kind = .native`, `.extensions` (as above — `.rs`), and two native-only fields: **`.module_root`** — the filename the author must place at the convention dir root (rust `mod.rs`, crystal `game.cr`, a future go `game.go`); its presence is a generate gate today (`NativeScriptsMissingModuleRoot`) — and **`.stage_subdir`** — the *plugin-crate-relative* directory the game's sources link over, replacing the shipped placeholder (rust `native/src/game`). rev 16's sketch carried `.module_root` but omitted `.stage_subdir`; **the decision rev 19 asks the owner to bless is adding `.stage_subdir` to the row.**
  - **Two forks for the epic owner.** (B1, `.stage_subdir` ownership) Either it is a row field (recommended — it is genuinely the *plugin's* crate layout, which differs per language/plugin and the assembler must know to place the link; a fixed convention would foreclose crystal/go layouts and third-party native plugins) or the assembler keeps a fixed convention and the plugin must conform (rejected — it re-hardcodes exactly what this section removes). (B2, the crate-layout gate) Today `stageNativeSources` fails with `NativeCrateLayoutMissing` when the staged package ships no crate parent for `.stage_subdir`, gated implicitly by the pinned plugin's age. **Recommend recasting it as the self-describing property**: a `.native` row *declaring* `.stage_subdir` is the pinned manifest's claim to ship that crate, so a missing crate is a plugin-packaging error naming the pin — no version compare, symmetric with how a missing `.declare` tool is handled (rev 17).
  - **Assembler consume contract.** For a `.kind = .native` splice: (1) collect the convention-dir `.extensions` sources and require `.module_root` at the root; (2) `linkDirAbs` the game convention dir over `<staged-pkg>/<stage_subdir>` (live view — edits flow without re-generate); (3) let the plugin's `.language_builds` steps (matched on `.language`) compile+link the staged crate. No `registerScript`, no `@embedFile`, no declare-vs-transpile branch beyond `.declare` (rev 17, which a native language may carry). The litmus native language (`.kind = .native` + `.module_root` + `.stage_subdir` + a `.language_builds` entry) stages and builds with zero assembler changes.
  - **The four splice touchpoints are NOT row data — they are the named codegen extension point.** #619 files "dep/flag/tap/tick wiring" under NATIVE, but grounding it in the source shows these four generated seams are **family-shared and language-invariant**, emitted identically for ANY active scripting splice (embed or native): the build-dep `.language = .<lang>` arg (`build_zig.zig`), the module-scope `const scripting = @import(...)` alias + `const scripting_enabled = true` flag (`render.zig`), the per-frame `engine.script_contract.drainEvents(&g)` **tap**, and the `scripting.Controller.tick(&g, scaled_dt)` **tick**. They vary only in that they bind fixed names of the **Script Runtime Contract v1** ABI (proposal 1: `registerScript`, `Controller.tick/setup`, `script_contract.drainEvents`) and the plugin import name. So the only per-language datum the native category actually moves is **`.kind`** (does native staging run at all) plus the two staging fields above — **the touchpoints themselves stay assembler-owned codegen** and are added to the honest-boundary residue (below), not the row. This is the one place #619's category description and the code diverge, and the RFC records the divergence deliberately.
...
- **Honest boundary.** Three pieces of assembler *codegen keyed to the scripting contract* are deliberately NOT table-lookup wiring, and stay **named extension points** (future: template-driven, plugin-supplied templates), not silent gaps: (1) typed-binding sidecar generation — the `labelle-components.d.ts` built from the registry; (2) the transpile-invocation codegen — the generated `tsconfig.json`, the emitted-file collision policy, and the `<tool> -p` shape (rev 18 moves the transpile toolchain's *fetch pins* to the manifest, but not this codegen); (3, rev 19) the **splice touchpoints** — the four generated seams bound to the Script Runtime Contract v1 ABI: the build-dep `.language` arg, the `const scripting = @import(...)` alias + `const scripting_enabled` flag, the `script_contract.drainEvents` tap, and the `Controller.tick` call. These are language-INVARIANT (identical for every splice, embed or native — see the native-wiring bullet), so they are contract-versioned codegen, not per-language row data; a contract v2 revises them centrally. v1 agnosticism deliberately scopes to collection, declare, embed registration, native staging, and toolchain fetch-pins; these three codegen surfaces are the acknowledged residue.
...
The litmus test this section must pass: **adding python = a labelle-scripting PR** — `src/python/` VM embed + `tools/declare-py` + a `.languages` row — **zero assembler changes**. With rev 19 the test now holds for BOTH families end-to-end: an embedded language adds a `.kind = .embedded` row (+ optional `.declare`/`.transpile`); a native language adds a `.kind = .native` row with `.module_root` + `.stage_subdir` (+ a `.language_builds` entry) — the assembler's collection, declare, embed-registration, transpile, and native-staging paths all read row fields only, and the sole assembler-owned residue is the three contract-versioned codegen surfaces named in the honest boundary.
...
- ~~**Toolchain-pin ownership (rev 16, proposal 7)**: assembler-owned fetch pins as the supply-chain control point, or manifest-owned with assembler-enforced mandatory hashes?~~ — **resolved (rev 18): option (b), manifest-with-mandatory-hashes.** The `.transpile` row carries the version + per-platform package + in-tarball binary path + mandatory sha512; the assembler refuses hash-less rows and verifies every byte before extraction. See the "Transpile fetch-pin contract" bullet in §7. (The tsconfig/collision/invocation codegen stays assembler-owned — the honest-boundary residue.)
- **Embed/native row spelling (rev 19, OPEN)** — the embed/registration and native-wiring capability contracts are pinned in §7 (rev 19), but three schema-spelling choices are left for the epic owner to bless before implementation: (A1) extension leading-dot form — `".rb"` (matches the shipped table + the `endsWith` scan; recommended) vs the sketches' dotless `"rb"`; (A2) `.extensions` plural (recommended, free forward-compat) vs singular `.extension`; and whether `.stage_subdir` joins the native row (recommended — plugin-owned crate layout). Also up for a nod: recasting the `.ts`→#586 untranspiled-source gate and the `NativeCrateLayoutMissing` crate gate as self-describing capability-presence checks (recommended, symmetric with the min-pin dissolution).
