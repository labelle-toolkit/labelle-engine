...
.languages = .{
    ...
    .{ .name = "typescript", .extensions = .{"ts"}, .kind = .embedded,
       ...
       // NO `.declare` for typescript — and this is NOT the same thing as
       // the SHIPPED ts d.ts codegen. Two distinct directions must not be
       // conflated: (a) the shipped `labelle-components.d.ts` codegen goes
       // registry → editor typings (the honest-boundary extension point
       // below), NOT source → schema; (b) a ts DECLARE-mode tool reading
       // `components/*.ts` → schema is future work (#773) and ships no
       // `tools/declare-ts` today — ts components are hand-written `.zig`.
       // The `.transpile` above is the only ts capability the row carries
       // now; the `.declare` field is simply omitted until (b) lands.
},
```
