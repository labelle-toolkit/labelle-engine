...
/// Note: The generated file will have a placeholder fingerprint that Zig
/// will auto-correct on first build attempt.
pub fn generateBuildZon(allocator: std.mem.Allocator, config: ProjectConfig) ![]const u8 {
    var buf: std.ArrayListUnmanaged(u8) = .{};
    const writer = buf.writer(allocator);

    try writer.writeAll(
        \\// Generated by labelle-engine
        \\// The fingerprint below is a placeholder - Zig will provide the correct one on first build
        \\.{
        \\
    );

    // Use a zero fingerprint - Zig will tell us the correct one
    try writer.writeAll("    .fingerprint = 0x0,\n");

    // Name (convert to identifier)
    try writer.print("    .name = .{s},\n", .{config.name});
    try writer.writeAll("    .version = \"0.1.0\",\n");
    try writer.writeAll("    .minimum_zig_version = \"0.15.2\",\n");

    // Dependencies - default to URL, user can change to .path for local dev
...
pub fn generateProject(allocator: std.mem.Allocator, project_path: []const u8) !void {
    // Load project config
