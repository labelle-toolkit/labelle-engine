// Example: Sokol backend hooks
//
// This file demonstrates the hook system with the Sokol backend.
// All hooks work the same as with raylib and SDL backends.
//
// Available hooks:
...
pub fn frame_start(_: @This(), info: engine.FrameInfo) void {
    ...
    if (info.frame_number % 60 == 0 and info.frame_number > 0) {
        std.log.info("[Sokol] Frame {d} (dt: {d:.1}ms)", .{
            info.frame_number,
            info.dt * 1000,
        });
    }
