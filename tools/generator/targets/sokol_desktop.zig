...
pub fn generateMainZigSokol(
    ...
    for (components, 0..) |name, i| {
        try zts.print(main_sokol_tmpl, "component_export", .{ component_type_names[i], name, component_type_names[i] }, writer);
    }
    ...
    if (has_plugin_components) {
        ...
        if (components.len == 0) {
            ...
            for (component_type_names) |type_name| {
                try zts.print(main_sokol_tmpl, "component_registry_multi_item", .{ type_name, type_name }, writer);
            }
        ...
        if (components.len == 0) {
            ...
            for (component_type_names) |type_name| {
                try zts.print(main_sokol_tmpl, "component_registry_item", .{ type_name, type_name }, writer);
            }
