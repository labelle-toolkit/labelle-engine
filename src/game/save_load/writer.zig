//! Save half of the save/load mixin — `saveGameState` (split from
//! `save_load_mixin.zig`, >1000-line rule; behavior-preserving).
...
pub fn Writer(comptime Game: type) type {
    ...
    return struct {
        ...
        pub fn saveGameState(self: *Game, filename: []const u8) !void {
                ...
                if (comptime core.getSavePolicy(T)) |policy| {
                    if (policy == .saveable or policy == .marker) {
                        ...
                        for (entities.items) |entity| {
                            const id = Sh.entityToU64(entity);
            ...
            if (self.current_scene_name) |scene_name| {
                ...
                try Sh.writeJsonString(writer, scene_name);
                try writer.writeAll(",\n");
            }
            ...
            for (entity_list.items, 0..) |id, idx| {
                ...
                if (comptime !Sh.isRegistered(Position)) {
                ...
                if (comptime !Sh.isRegistered(Parent)) {
                    if (self.active_world.ecs_backend.getComponent(entity, Parent)) |parent| {
                        if (!first_comp) try writer.writeAll(",");
                        try writer.writeAll("\n        \"Parent\": {\"entity\": ");
                        try writer.print("{d}", .{Sh.entityToU64(parent.entity)});
                ...
                if (comptime !Sh.isRegistered(PrefabInstance)) {
                    if (self.active_world.ecs_backend.getComponent(entity, PrefabInstance)) |pi| {
                        if (!first_comp) try writer.writeAll(",");
                        try writer.writeAll("\n        \"PrefabInstance\": {\"path\": ");
                        try Sh.writeJsonString(writer, pi.path);
                        try writer.writeAll(", \"overrides\": ");
                        try Sh.writeJsonString(writer, pi.overrides);
                        try writer.writeAll("}");
                        first_comp = false;
                    }
                }
                ...
                if (comptime !Sh.isRegistered(PrefabChildT)) {
                    if (self.active_world.ecs_backend.getComponent(entity, PrefabChildT)) |pc| {
                        if (!first_comp) try writer.writeAll(",");
                        try writer.writeAll("\n        \"PrefabChild\": {\"root\": ");
                        try writer.print("{d}", .{Sh.entityToU64(pc.root)});
                        try writer.writeAll(", \"local_path\": ");
                        try Sh.writeJsonString(writer, pc.local_path);
                        try writer.writeAll("}");
                        first_comp = false;
                    }
                }
