//! ImGui bgfx Adapter
//!
//! GUI backend using Dear ImGui with bgfx/GLFW rendering.
//! Uses zgui for Zig bindings to ImGui.
//! Note: bgfx has its own imgui integration that could be used alternatively.
//!
//! NOTE: Full ImGui support with bgfx is not yet available.
//! bgfx doesn't expose a GLFW window handle needed by zgui's backends.
...
pub fn beginFrame(self: *Self) void {
    ...
    if (!self.warned) {
        std.log.warn("imgui_bgfx: ImGui not available with bgfx backend", .{});
        std.log.warn("imgui_bgfx: Use zgpu backend for ImGui support", .{});
        self.warned = true;
    }
...
pub fn endFrame(self: *Self) void {
    _ = self;
    // Skip - nothing to render with bgfx backend
}
