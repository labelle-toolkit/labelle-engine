...
pub fn generateMainZig(
    ...
    return switch (config.backend) {
        ...
        .sokol => generateMainZigSokol(allocator, config, prefabs, components, scripts),
    };
}

/// Generate raylib-style main.zig (polling loop)
fn generateMainZigRaylib(
    ...
    // Write common imports and registries
    try writeImportsAndRegistries(writer, config, prefabs, components, scripts);

    // Main function (raylib polling style)
    try writer.writeAll(
        \\pub fn main() !void {
...
/// Generate sokol-style main.zig (callback-based with sokol_app)
fn generateMainZigSokol(
    ...
    // Header with sokol imports - minimal template for sokol backend
    try writer.print(
        \\// Generated by labelle-engine for project: {s}
        \\// Do not edit manually - regenerate with: zig build generate
        \\//
        ...
        \\}}
        \\
    , .{ config.name, config.window.width, config.window.height, config.window.title });

    return buf.toOwnedSlice(allocator);
}

/// Write common imports and registries (shared between raylib and sokol templates)
fn writeImportsAndRegistries(
    writer: anytype,
    config: ProjectConfig,
    prefabs: []const []const u8,
    components: []const []const u8,
