...
test "query: basic each with two components" {
    ...
    registry.addComponent(e1, Position{ .x = 10, .y = 20 });
    registry.addComponent(e1, Velocity{ .dx = 1, .dy = 2 });

    const e2 = registry.createEntity();
    registry.addComponent(e2, Position{ .x = 30, .y = 40 });
    registry.addComponent(e2, Velocity{ .dx = 3, .dy = 4 });
    ...
    // Verify mutations
    const pos1 = registry.getComponent(e1, Position).?;
    try testing.expectEqual(@as(f32, 11), pos1.x);
    try testing.expectEqual(@as(f32, 22), pos1.y);

    const pos2 = registry.getComponent(e2, Position).?;
...
test "query: single component" {
    ...
    const e1 = registry.createEntity();
    registry.addComponent(e1, Position{ .x = 5, .y = 10 });

    const e2 = registry.createEntity();
    registry.addComponent(e2, Position{ .x = 15, .y = 20 });
...
test "query: entities without matching components are skipped" {
    ...
    const e2 = registry.createEntity();
    registry.addComponent(e2, Position{ .x = 100, .y = 200 });
